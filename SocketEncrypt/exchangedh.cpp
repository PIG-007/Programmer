...
#include "ui_exchangedh.h"
#include <QStyleFactory>
#include "DH.h"

exchangeDH::exchangeDH(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::exchangeDH)
{
    ui->setupUi(this);
    qApp->setStyle(QStyleFactory::create("fusion"));
    setWindowTitle("DH Server");
    initServer();
}
...
void exchangeDH::initServer()
{
    ...
    connect(server,&QTcpServer::newConnection,this,[this]{
        ...
        while(server->hasPendingConnections())
        {
            ...
            connect(socket,&QTcpSocket::readyRead,[this,socket]{
                ...
                DH_clientKey = recv_text.toUInt();
                ui->textRecv->appendPlainText(QString("[%1:%2]")
                                     .arg(socket->peerAddress().toString())
                                     .arg(socket->peerPort()));
                ui->textRecv->appendPlainText(recv_text);
                ui->clientKeyRecv->appendPlainText("DH_clientKey:"+QString::number(DH_clientKey)+QString("[%1:%2]")
                                                   .arg(socket->peerAddress().toString())
                                                   .arg(socket->peerPort()));
            });
    ...
    connect(ui->btnSend,&QPushButton::clicked,[this]{
        ...
        if(!server->isListening())
            return;
        //将P G server_key文本发送给客户端
        const QByteArray send_data= ("DH_P:" + QString::number(DH_p) + "\n"+
                                        "DH_G:" + QString::number(DH_g) + "\n"+
                                        "DH_serverKey:" + QString::number(DH_serverKey) + "\n").toUtf8();
        //数据为空就返回
...
void exchangeDH::on_genKeyBtn_clicked()
{
    genertateDHKey(DH_p, DH_g, DH_serverKey, gen_b);

//    ui->textRecv->appendPlainText(QString::number(DH_p));
//    ui->textRecv->appendPlainText(QString::number(DH_g));
//    ui->textRecv->appendPlainText(QString::number(DH_serverKey));
    //ui->textRecv->appendPlainText(QString::number(DH_b));
    ui->pLabel->setText("P:"+QString::number(DH_p));
    ui->gLabel->setText("G:"+QString::number(DH_g));
    ui->serverKeyLabel->setText("Server Key:"+QString::number(DH_serverKey));
...
void exchangeDH::on_genComKeyBtn_clicked()
{
    switchKey(DH_p, DH_g, DH_serverKey, DH_clientKey, DH_ga, DH_gb, DH_comKey, DH_comKey);
    ui->comKeyEdit->appendPlainText("DH_comKey:"+QString::number(DH_comKey));
}
